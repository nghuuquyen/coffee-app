@if (isset($href))
    <a href="{{ $href }}" target="{{ $target ?? '_blank' }}"
        {{ $attributes->merge(['class' => 'flex bg-primary-600 hover:bg-primary-700 text-sm text-on-primary-50 active:translate-y-1 rounded-lg lg:rounded-2xl px-4 py-2 flex-shrink-0 w-auto items-center justify-center']) }}>
